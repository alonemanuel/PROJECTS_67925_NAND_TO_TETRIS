            ...
            while self.idx < len(self.code):
                if self.mode == CODE:
                    ...
                    if self.modes_function[self.mode]():
                        # self.next_token_type = KEYWORD if self.temp_token in self.keywords else self.mode
    ...
    def set_mode(self):
        ...
        if self.code[self.idx] == '/':
            if len(self.code) > self.idx + 1 and self.code[self.idx + 1].startswith('/'):
                self.mode = COMMENT
                return
            elif len(self.code) > self.idx + 1 and (self.code[self.idx + 1:].startswith('**') or self.code[self.idx + 1:].startswith('*')):
                self.mode = COMMENT_BLOCK
                return
        if self.code[self.idx].isalpha() or self.code[self.idx] == '_':
